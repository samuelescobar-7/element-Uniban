...
# =========================
# % TER — Requerimientos funcionales
# =========================
def construir_tabla_ter_funcional(detalles_globales, nombres_proveedores):
    """
    Construye una tabla con el % de respuestas TER de la columna F, por hoja
    funcional (proceso) y por proveedor.
    ...
    for hoja, lista_dfs in detalles_globales.items():
        ...
        for prov in nombres_proveedores:
            ...
            total = len(df_prov)
            total_ter = (df_prov["Resp_F"].map(normalizar) == "TER").sum()
            fila[prov] = round((total_ter / total) * 100, 2) if total else 0.0

        filas.append(fila)

    columnas_orden = ["proceso"] + nombres_proveedores
    df_ter = pd.DataFrame(filas)
    for c in columnas_orden:
        if c not in df_ter.columns:
            df_ter[c] = 0.0
    df_ter = df_ter[columnas_orden]
    return df_ter


def escribir_hoja_ter(writer, df_ter_func, nombres_proveedores, nombre_hoja="% TER"):
    """Escribe la hoja % TER con el bloque Requerimientos funcionales."""
    if df_ter_func is None or df_ter_func.empty:
        return
    ...
    df_escribir = agrupar_df_por_categoria(df_ter_func, columna_hoja="proceso")
    for _, row in df_escribir.iterrows():
        for col_idx, col_name in enumerate(columnas_func, start=1):
            val = row.get(col_name, "")
            if col_name != "proceso" and isinstance(val, (int, float)) and not isinstance(val, bool):
...
# =========================
# % NO/BLANCO/OTROS — Requerimientos funcionales
# =========================
def construir_tabla_no_blanco_otros_funcional(detalles_globales, nombres_proveedores):
    """
    Construye una tabla con el % de respuestas NO, blanco u otros de la columna F,
    por hoja funcional (proceso) y por proveedor.
    ...
    for hoja, lista_dfs in detalles_globales.items():
        ...
        for prov in nombres_proveedores:
            ...
            total = len(df_prov)
            total_no_blanco_otros = (~df_prov["Resp_F"].map(normalizar).isin(respuestas_excluidas)).sum()
            fila[prov] = round((total_no_blanco_otros / total) * 100, 2) if total else 0.0

        filas.append(fila)

    columnas_orden = ["proceso"] + nombres_proveedores
    df_no_blanco_otros = pd.DataFrame(filas)
    for c in columnas_orden:
        if c not in df_no_blanco_otros.columns:
            df_no_blanco_otros[c] = 0.0
    df_no_blanco_otros = df_no_blanco_otros[columnas_orden]
    return df_no_blanco_otros


def escribir_hoja_no_blanco_otros(writer, df_no_blanco_otros_func, nombres_proveedores,
                                  nombre_hoja="% NO-BLANCO-OTROS"):
    """Escribe la hoja % NO-BLANCO-OTROS con el bloque Requerimientos funcionales."""
    if df_no_blanco_otros_func is None or df_no_blanco_otros_func.empty:
        return
    ...
    df_escribir = agrupar_df_por_categoria(df_no_blanco_otros_func, columna_hoja="proceso")
    for _, row in df_escribir.iterrows():
        for col_idx, col_name in enumerate(columnas_func, start=1):
            val = row.get(col_name, "")
            if col_name != "proceso" and isinstance(val, (int, float)) and not isinstance(val, bool):
...
def construir_tabla_no_f_si_g_funcional(detalles_globales, nombres_proveedores):
    """
    Construye una tabla con el % de filas donde la columna F tiene respuesta NO
    y, en esa misma fila, la columna G tiene respuesta SI, por hoja funcional
    y por proveedor.

    Devuelve un DataFrame con columnas:
    Proceso | % NO F y SI G <prov1> | ... | % NO F y SI G <provN>
    """
    if not detalles_globales:
        return None
    ...
    for hoja, lista_dfs in detalles_globales.items():
        ...
        for prov in nombres_proveedores:
            ...
            total = len(df_prov)
            coincidencias = (
                (df_prov["Resp_F"].map(normalizar) == "NO")
                & (df_prov["Resp_G"].map(normalizar) == "SI")
            ).sum()
            fila[col_name] = round((coincidencias / total) * 100, 2) if total else 0.0

        filas.append(fila)

    columnas_orden = ["Proceso"] + [f"% NO F y SI G {prov}" for prov in nombres_proveedores]
    df_no_si = pd.DataFrame(filas)
    for c in columnas_orden:
        if c not in df_no_si.columns:
            df_no_si[c] = 0.0
    df_no_si = df_no_si[columnas_orden]
    return df_no_si


def construir_tabla_no_d_si_e_no_funcional(detalles_globales_nf, nombres_proveedores):
    """
    Construye una tabla con el % de filas donde la columna D tiene respuesta NO
    y, en esa misma fila, la columna E tiene respuesta SI, por hoja no funcional
    y por proveedor.

    Devuelve un DataFrame con columnas:
    Proceso | % NO D y SI E <prov1> | ... | % NO D y SI E <provN>
    """
    if not detalles_globales_nf:
        return None
    ...
    for hoja, lista_dfs in detalles_globales_nf.items():
        ...
        for prov in nombres_proveedores:
            ...
            total = len(df_prov)
            coincidencias = (
                (df_prov["Resp_D"].map(normalizar) == "NO")
                & (df_prov["Resp_E"].map(normalizar) == "SI")
            ).sum()
            fila[col_name] = round((coincidencias / total) * 100, 2) if total else 0.0

        filas.append(fila)

    columnas_orden = ["Proceso"] + [f"% NO D y SI E {prov}" for prov in nombres_proveedores]
    df_no_si = pd.DataFrame(filas)
    for c in columnas_orden:
        if c not in df_no_si.columns:
            df_no_si[c] = 0.0
...
def escribir_hoja_no_f_si_g(writer, df_no_si_func, nombres_proveedores_func,
                             df_no_si_nf, nombres_proveedores_nf,
                             nombre_hoja="% de NO (F) y SI (G)"):
    """Escribe la hoja de validación inversa con los bloques funcional y no funcional."""
    hay_func = df_no_si_func is not None and not df_no_si_func.empty
    hay_nf = df_no_si_nf is not None and not df_no_si_nf.empty
    if not hay_func and not hay_nf:
        return

    columnas_func = ["Proceso"] + [f"% NO F y SI G {prov}" for prov in nombres_proveedores_func]
    columnas_nf = ["Proceso"] + [f"% NO D y SI E {prov}" for prov in nombres_proveedores_nf]

    ws = writer.book.create_sheet(nombre_hoja)
    fila_actual = 1

    fila_actual = _escribir_bloque_blancos(
        ws, df_no_si_func, "Requerimientos funcionales", columnas_func, fila_actual, agrupar=True
    )
    fila_actual = _escribir_bloque_blancos(
        ws, df_no_si_nf, "Requerimientos no funcionales", columnas_nf, fila_actual, agrupar=False
    )
...
if st.session_state["archivos_cargados"]:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        df_ter_func = construir_tabla_ter_funcional(detalles_globales, nombres_proveedores)
        escribir_hoja_ter(writer, df_ter_func, nombres_proveedores)
        df_no_blanco_otros_func = construir_tabla_no_blanco_otros_funcional(detalles_globales, nombres_proveedores)
        escribir_hoja_no_blanco_otros(writer, df_no_blanco_otros_func, nombres_proveedores)
        ...
        df_no_si_func = construir_tabla_no_f_si_g_funcional(detalles_globales, nombres_proveedores)
        df_no_si_nf = construir_tabla_no_d_si_e_no_funcional(detalles_globales_nf, nombres_proveedores)
    ...
    with pd.ExcelWriter(buffer_validaciones, engine="openpyxl") as writer_val:
        ...
        escribir_hoja_no_f_si_g(
            writer_val, df_no_si_func, nombres_proveedores, df_no_si_nf, nombres_proveedores,
            nombre_hoja="% de NO (F) y SI (G)"
        )
